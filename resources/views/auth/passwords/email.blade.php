						...
						<div class="col-sm-7 col-sm-offset-3 col-md-6 col-md-offset-2 main content-start">
							<div class="content-form-login-page-school-plugin md-form">
								<form class="form-horizontal" method="POST" action="{{ url('/password/forgot') }}">
									{{ csrf_field() }}
									<h3 class="fw-bold">{{ trans('message.Forgot Password') }}</h3>
									<!-- <p class="login-username">
										<label for="forgot_pass"> Email </label>
										<input id="email_reset" type="email" class="form-control email" name="email" value="{{ old('email') }}" placeholder="Enter Valid E-mail" required>
									...
									<p class="login-username mt-3">
										...
										<input type="text" name="email" id="email_reset" autocomplete="off" class="input" value="" size="20" required>
										@if ($errors->has('email'))
									...
									<p class="col-md-12 col-sm-12 col-xs-12">
										<button type="submit" class="btn btn-success reset">
											<i class="fa fa-btn fa-envelope"></i> {{ trans('message.SUBMIT') }}
										</button>
									</p>
								</form>
							</div>
