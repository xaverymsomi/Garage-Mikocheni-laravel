          ...
          <div class="x_content">
            <ul class="nav nav-tabs bar_tabs"
              ...
              <li role="presentation"
                class="active"><a href="{!! url('/labor_hour/list') !!}"><span class="visible-xs"></span> <i
                    class="fa fa-list fa-lg">&nbsp;</i><b>{{ trans('List Of Inspection Labour Hour') }}</b></span></a></li>

              <li role="presentation"
                class=""><a href="{!! url('/labor_hour/add') !!}"><span class="visible-xs"></span> <i
                    class="fa fa-plus-circle fa-lg">&nbsp;</i>{{ trans('Add Hours') }}</span></a></li>
              ...
              <tbody>
                ...
                @foreach ($o_type_point as $o_type_points)
                  <tr>
                    ...
                    <td>{{ $o_type_points->hours }}</td>

                    <td>

                      <a href="{!! url('/labor_hour/list/edit/' . $o_type_points->id) !!}"><button type="button"
                          class="btn btn-round btn-success">{{ trans('message.Edit') }}</button></a>
                          <form action="{!! url('/labor_hour/list/delete/' . $o_type_points->id) !!}" method="POST" class="delete-form" style="display: inline;">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn btn-round btn-danger sa-warning">{{ trans('message.Delete') }}</button>
                        </form>
                      {{-- <a url="{!! url('/labor_hour/list/delete/' . $o_type_points->id) !!}"
