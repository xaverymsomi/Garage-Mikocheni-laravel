...
<body id="app-layout" class="nav-md">
   <div class="container body">
    <div class="main_container">
            ...
            <div id="sidebar-menu" class="main_menu_side hidden-print main_menu">
              <div class="menu_section">
				...
				<?php $userid=Auth::User()->id;?>
                  ...
                  <li><a href="{!! url('/') !!}"><i class="fa fa-home"></i> {{ trans('message.Dashboard')}} </a> </li>
				  
				<?php $userid=Auth::User()->id;?>
				@if (getAccessStatusUser('Inventory',$userid)=='yes')
				   <li><a><i class="fa fa-user image_icon"></i> {{ trans('message.Inventory')}} <span class="fa fa-chevron-down"></span></a>
                    <ul class="nav child_menu">
						<li><a href="{!! url('/supplier/list')!!}">{{ trans('message.Supplier')}}</a></li>
						<li><a href="{!! url('/product/list') !!}">{{ trans('message.Product')}}</a></li>
						<li><a href="{!! url('/purchase/list')!!}">{{ trans('message.Purchase')}}</a></li>
						<li><a href="{!! url('/stoke/list')!!}">{{ trans('message.Stock')}}</a></li>
                    </ul>
				  </li>
				 @endif
				 
                  <li><a><i class="fa fa-edit"></i> {{ trans('message.Users')}} <span class="fa fa-chevron-down"></span></a>
                    <ul class="nav child_menu">
					<?php $userid=Auth::User()->id;?>
				     @if (getAccessStatusUser('Customers',$userid)=='yes')
                      <li><a href="{!! url('/customer/list')!!}">{{ trans('message.Customers')}}</a></li>
				     @endif
					 
					 <?php $userid=Auth::User()->id;?>
				     @if (getAccessStatusUser('Employee',$userid)=='yes')
                      <li><a href="{!! url('/employee/list')!!}">{{ trans('message.Employee')}}</a></li>
				     @endif
					 <?php $userid=Auth::User()->id;?>
				     @if (getAccessStatusUser('Support Staff',$userid)=='yes')
					  <li><a href="{!! url('/supportstaff/list')!!}">{{ trans('message.Support Staff')}}</a></li>
				     @endif
					 
					 <?php $userid=Auth::User()->id;?>
				     @if (getAccessStatusUser('Accountant',$userid)=='yes')
                      <li><a href="{!! url('/accountant/list')!!}">{{ trans('message.Accountant')}}</a></li>
				     @endif
				...
				@if (getAccessStatusUser('Vehicle',$userid)=='yes')
                  <li><a><i class="fa fa-motorcycle"></i> {{ trans('message.Vehicle')}} <span class="fa fa-chevron-down"></span></a>
                    <ul class="nav child_menu">
                      <li><a href="{!! url('/vehicle/list') !!}">{{ trans('message.List Vehicle')}}</a></li>
                      <li><a href="{!! url('/vehicletype/list') !!}">{{ trans('message.List Vehicle Type')}}</a></li>
                      <li><a href="{!! url('/vehiclebrand/list') !!}">{{ trans('message.List Vehicle Brand')}}</a></li>
                    </ul>
                  </li>
                 @endif 
                 @endif 
				 @if(getActiveCustomer($userid)=='no')
					@if (getAccessStatusUser('Vehicle',$userid)=='yes')
						<li><a href="{!! url('/vehicle/list') !!}"><i class="fa fa-motorcycle"></i> {{ trans('message.Vehicle')}} </a></li>
					@endif 
                 @endif 
				<?php $userid=Auth::User()->id;?>
				@if (getAccessStatusUser('Services',$userid)=='yes')
                  <li><a href="{!! url('/service/list') !!}"><i class="fa fa-slack image_icon"></i> {{ trans('message.Services')}} </a> </li>
				@endif

				<?php $userid=Auth::User()->id;?>
				@if (getAccessStatusUser('Invoice',$userid)=='yes')
                  <li><a href="{!! url('/invoice/list') !!}" ><i class="fa fa-file-text-o"></i> {{ trans('message.Invoice')}} </a>  </li>
				@endif

				
				<?php $userid=Auth::User()->id;?>
				@if (getAccessStatusUser('Job Card',$userid)=='yes')
                  <li><a><i class="fa fa-table"></i> {{ trans('message.Job Card')}} <span class="fa fa-chevron-down"></span></a>
                    <ul class="nav child_menu">
                      <li><a href="{!! url('/jobcard/list')!!}">{{ trans('message.Job Card')}}</a></li>
                      <li><a href="{!! url('/gatepass/list')!!}">{{ trans('message.Gate Pass')}}</a></li>
				...
				@if (getAccessStatusUser('Colors',$userid)=='yes')
				 <li><a href="{!! url('/color/list') !!}"><i class="fa fa-bar-chart-o"></i> {{ trans('message.Colors')}}</a></li>
				 @endif 
				  
				  
				 <?php $userid=Auth::User()->id;?>
				@if (getAccessStatusUser('Account & Tax Rates',$userid)=='yes')
				  <li><a><i class="fa fa-tasks image_icon"></i>{{ trans('message.Account & Tax Rates')}} <span class="fa fa-chevron-down"></span></a>
                    <ul class="nav child_menu">
                      <li><a href="{!! url('/taxrates/list') !!}">{{ trans('message.List Tax Rates')}}</a></li>
                      <li><a href="{!! url('/payment/list') !!}">{{ trans('message.List Payment')}} </a></li>
                    </ul>
                  </li>
				@endif
				...
				@if (getAccessStatusUser('Sales',$userid)=='yes')
					@if(getActiveCustomer($userid)=='yes' || getActiveEmployee($userid)=='yes')
						<li><a href="{!! url('/sales/list') !!}"><i class="fa fa-tty image_icon"></i>{{ trans('message.Sales')}} </a> </li>
					@else
						<li><a href="{!! url('/sales/list') !!}"><i class="fa fa-tty image_icon"></i>{{ trans('message.Purchase')}} </a> </li>
					@endif
				@endif
				
				<?php $userid=Auth::User()->id;?>
				@if (getAccessStatusUser('Government Management',$userid)=='yes')
				  <li><a href="{!! url('/rto/list') !!}"><i class="fa fa-clone"></i>{{ trans('message.Compliance')}}</br> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; {{ trans('message.Management')}}</a>
                  </li>
				@endif
				
				<?php $userid=Auth::User()->id;?>
				@if (getAccessStatusUser('Account',$userid)=='yes')
				  <li><a><i class="fa fa-calculator"></i> {{ trans('message.Account')}} <span class="fa fa-chevron-down"></span></a>
                    <ul class="nav child_menu">
                      <li><a href="{!! url('/income/list')!!}">{{ trans('message.Income')}}</a></li>
                      <li><a href="{!! url('/expense/list')!!}">{{ trans('message.Expenses')}}</a></li>
                    </ul>
                  </li>
				@endif
				...
				@if (getAccessStatusUser('Report',$userid)=='yes')
				  <li><a href="{!! url('/report/salesreport') !!}"><i class="fa fa-bar-chart-o"></i>{{ trans('message.Report')}} </a></li>
				 @endif
				 
				<?php $userid=Auth::User()->id;?>
				@if (getAccessStatusUser('Mail Formates',$userid)=='yes')
				  <li><a href="{!! url('/mail/mail') !!}"><i class="fa fa-envelope"></i>{{ trans('message.Email Template')}}</a></li>
				@endif
				 
				<?php $userid=Auth::User()->id;?>
				@if (getAccessStatusUser('Custom Field',$userid)=='yes')
				  <li><a href="{!! url('/setting/custom/list') !!}"><i class="fa fa-user"></i>{{ trans('message.Custom Field')}}</a> </li>
				@endif
				
				<?php $userid=Auth::User()->id;?>
				@if (getAccessStatusUser('Observation library',$userid)=='yes')
				 <li><a href="{!! url('/observation/list') !!}" ><i class="fa fa-universal-access"></i>{{ trans('message.Observation library')}}</a></li> 
				@endif
                </ul>
              </div>
            </div>
	   ...
	   <footer>
          <div>
             <span class="footerbottom">{{ trans('message.All rights reserved by Garage System.')}}</span>
          </div>
